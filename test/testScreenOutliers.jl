## --- Create a test dataset 
    ...
    bulk_cats = get_cats(false, 10)[2]
    [bulk_cats[k] .= rand(Bool, 10) for k in keys(bulk_cats)]


## --- Screen outliers and check that the function mutated the structure
    screen_outliers!(bulk, bulk_cats)
...
##--- Test trace elements
    ...
    tmin = 0        # Minimum allowable trace concentration is 0 wt.%
    ...
    @test all(isnan.(bulk.Ce[.!(tmin .< bulk_initial.Ce .< tmax)]))
    ...
    @test all(isnan.(bulk.F[.!(tmin .< bulk_initial.F .< tmax)]))
    ...
    @test all(isnan.(bulk.Ho[.!(tmin .< bulk_initial.Ho .< tmax)]))
    ...
    @test all(isnan.(bulk.Nd[.!(tmin .< bulk_initial.Nd .< tmax)]))
    @test all(isnan.(bulk.Pb[.!(tmin .< bulk_initial.Pb .< tmax)]))
    @test all(isnan.(bulk.Pr[.!(tmin .< bulk_initial.Pr .< tmax)]))
    ...
    @test all(isnan.(bulk.Sm[.!(tmin .< bulk_initial.Sm .< tmax)]))
    ...
    # Anything we did unit conversions on need tested separately
    tmax = 0.1
    @test all(isnan.(bulk.Au[.!(tmin .< bulk.Au .< tmax)]))
    @test all(isnan.(bulk.Ir[.!(tmin .< bulk.Ir .< tmax)]))
    @test all(isnan.(bulk.Os[.!(tmin .< bulk.Os .< tmax)]))
    @test all(isnan.(bulk.Pd[.!(tmin .< bulk.Pd .< tmax)]))
    @test all(isnan.(bulk.Pt[.!(tmin .< bulk.Pt .< tmax)]))
    @test all(isnan.(bulk.Re[.!(tmin .< bulk.Re .< tmax)]))

    # Sulfur can be large, from pyrite
    @test all(isnan.(bulk.S[.!(tmin .< bulk_initial.S .< 28)]))
